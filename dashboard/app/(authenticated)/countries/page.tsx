...
import { deleteCountry, listCountries, type Country } from '@/lib/timegate/countries'
...
export default function CountriesPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError('')
    try {
      const res = await listCountries({ limit: 100 })
      setData(res.data)
    } catch (err) {
  ...
  return (
    <div>
      <PageHeader
        breadcrumbs={[{ label: 'Plateforme' }, { label: 'Pays' }]}
        action={<AddPageLink href="/countries/new" label="Ajouter un pays" />}
      />
      {error && (
      ...
      <DataTable
        ...
        actions={(row) => (
          <ActionButtons
            viewHref={`/countries/${row.id}`}
            editHref={`/countries/${row.id}/edit`}
            onDelete={() => void deleteCountry(row.id).then(() => load())}
