...
import { getPlatformStats } from '@/lib/api/organizations'
        ...
        <>
          ...
          <div className="tg-card border border-slate-200/80 shadow-2xs rounded-xl dark:border-border-dark p-5">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                { href: '/organizations', label: 'Organisations', icon: 'fa-building' },
                { href: '/plans', label: 'Plans', icon: 'fa-layer-group' },
                { href: '/subscriptions', label: 'Abonnements', icon: 'fa-credit-card' },
                { href: '/platform-settings', label: 'Paramètres', icon: 'fa-sliders' },
              ].map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
